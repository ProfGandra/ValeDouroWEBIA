...
const STORAGE_KEY='valedouro.audio.v2';
...
async function play(key){
  ...
  try{
    ...
    console.warn('Trilha não pôde iniciar automaticamente:',err);
    updateUI('Clique em Som ambiente para iniciar');
    return false;
  }
}
...
function resume(){
  unlocked=true;
  syncPlayer();
  if(currentKey&&settings.enabled)return play(currentKey);
  updateUI();
...
function installUI(){
  ...
  style.textContent='.vale-audio-controls{position:fixed;left:12px;bottom:118px;z-index:9997;width:224px;padding:9px 10px;border:1px solid rgba(201,164,92,.42);border-radius:12px;background:rgba(18,15,12,.88);backdrop-filter:blur(8px);box-shadow:0 5px 18px rgba(0,0,0,.32);color:#e9dfd0;font-size:11px}.vale-audio-toggle{width:100%;display:flex;align-items:center;gap:7px;border:0;background:transparent;color:inherit;font:inherit;font-weight:700;text-align:left;cursor:pointer;padding:1px 0 6px}.vale-audio-toggle:hover{color:#f0c875}.vale-audio-toggle[aria-pressed="false"]{opacity:.7}.vale-audio-volume{display:flex;align-items:center;gap:7px}.vale-audio-volume input{flex:1;min-width:0;accent-color:#c9a45c;cursor:pointer}.vale-audio-volume span{width:34px;text-align:right;font-variant-numeric:tabular-nums;color:#cdbfae}.vale-audio-now{margin-top:5px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:#a99d8c;font-size:10px}@media(max-width:800px){.vale-audio-controls{left:6px;bottom:94px;width:176px;padding:7px 8px}.vale-audio-text{font-size:10px}.vale-audio-now{display:none}}@media print{.vale-audio-controls{display:none!important}}';
  ...
  range.addEventListener('input',e=>setVolume((Number(e.target.value)||0)/100));
  range.addEventListener('change',e=>setVolume((Number(e.target.value)||0)/100));
  updateUI();
}

function updateUI(note=''){
  const toggleEl=document.getElementById('valeAudioToggle');
  ...
  toggleEl.title=settings.enabled?'Silenciar trilha sonora':'Ativar trilha sonora';
...
function unlockFromGesture(){unlocked=true;syncPlayer();if(settings.enabled&&currentKey&&player.paused)play(currentKey)}
function init(){installUI();installAIBridge();hookShow();syncPlayer();if(activeScreen()==='opening')play('main_menu');document.addEventListener('pointerdown',unlockFromGesture,{once:true,capture:true});document.addEventListener('keydown',unlockFromGesture,{once:true,capture:true})}

window.ValeAudio={tracks:TRACKS,play,stop,pause,resume,toggle,setEnabled,setVolume,getState:()=>({enabled:settings.enabled,volume:settings.volume,currentKey,unlocked,paused:player.paused,muted:player.muted,actualVolume:player.volume}),inferFromNarrative,player};
