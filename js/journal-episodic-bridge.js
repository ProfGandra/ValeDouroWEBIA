...
function classify(action){
  ...
  if(/\b(ataco|golpeio|esfaqueio|corto|estoco|agredo|desfiro|acerto|firo)\b/.test(t)||/sac\w*\s+(minha\s+)?espada.*(golpe|atac|cort|estoc)/.test(t))return {type:'assault',title:'Sangue pela minha mão',body:'Hoje escolhi erguer minha arma contra outra pessoa. O resultado ainda poderia ser incerto, mas a decisão foi minha.'};
  if(/\b(furto|roubo|roubar|assalto|assaltar|subtraio|tomo para mim)\b/.test(t))return {type:'theft',title:'Aquilo que não era meu',body:'Hoje decidi tomar para mim aquilo que pertencia a outro. O resultado veio depois; a escolha foi minha.'};
