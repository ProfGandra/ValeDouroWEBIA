...
function rest(c,type,hours){ensure(c);type=RULES[type]?type:classify(hours);if(!type)return null;const r=RULES[type],before={hp:Number(c.hp||0),mana:c.mana??null,fatigue:c.fatigue,arcaneFatigue:c.arcaneFatigue};c.fatigue=clamp(c.fatigue+r.fatigue,0,100);if(c.manaMax!=null){const gain=Math.max(1,Math.floor(Number(c.manaMax)*r.mana));c.mana=clamp(Number(c.mana||0)+gain,0,Number(c.manaMax));}c.arcaneFatigue=Math.max(0,c.arcaneFatigue+r.arcaneFatigue);const cap=injuryCap(c),natural=Math.floor(Number(c.hpMax||0)*r.hp);c.hp=clamp(Number(c.hp||0)+natural,0,cap);c.lastRest={type,label:r.label,hours:Number(hours)||r.minHours,at:new Date().toISOString()};persist();try{window.ValeInjuries?.advanceTime?.(c,Number(hours)||r.minHours);}catch{}return {type,label:r.label,before,after:{hp:c.hp,mana:c.mana??null,fatigue:c.fatigue,arcaneFatigue:c.arcaneFatigue},hpCap:cap,activeInjuries:c.injuries.filter(x=>x.active!==false).length};}
