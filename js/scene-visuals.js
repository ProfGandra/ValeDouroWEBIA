...
function inferSceneFromNarrative(text,q=campaignQuest()){
  if(q!=='QST-001')return null;
  const t=normalizeText(text);if(!t)return null;
  const rules=[
    ['LOC-007',[/depressao rochosa/,/local da queda/,/aprendiz (?:esta |foi )?caido/,/encontr(?:a|am|ou) o aprendiz/,/ferid[oa] entre (?:as )?pedras/]],
    ['LOC-006',[/adentr(?:a|ando|aram|ou).{0,35}floresta/,/densa floresta/,/interior da floresta/,/mata fechada/,/dentro da floresta/,/est[aá] na floresta/,/estao na floresta/]],
    ['LOC-005',[/trilha.{0,35}bandid/,/rastros? dos bandid/,/rota.{0,25}bandid/,/vegetacao pisoteada.{0,80}pegadas/]],
    ['LOC-004',[/local da caravana/,/caravana (?:parada|encontrada|imobilizada)/,/carroca.{0,40}(?:roda|eixo).{0,30}(?:danific|quebrad|imobiliz)/,/sobreviventes.{0,60}caravana/]],
    ['LOC-003',[/trecho acidentado/,/estrada (?:fica|torna-se|esta) irregular/,/sulcos profundos/,/terreno acidentado.{0,50}estrada/]],
    ['LOC-002',[/ultimo ponto confirmado/,/ultimo local confirmado/,/ultimo ponto de passagem/,/marcas recentes de rodas.{0,40}ferraduras/]],
    ['LOC-001',[/saida de valedouro/,/portao de valedouro/,/deixam valedouro/,/partem de valedouro/]]
  ];
  for(const [loc,patterns] of rules)if(patterns.some(r=>r.test(t)))return loc;
  return null;
}
...
window.ValeSceneVisuals={assets:ASSETS,read:ensureState,setScene,render,reset,inferSceneFromNarrative};
...
window.fetch=async function(input,init){
  ...
  try{
    ...
    for(const field of ['reply','text'])if(typeof data?.[field]==='string'){
      ...
      data[field]=data[field].replace(/\s*\[\[SCENE:(LOC-\d{3})\]\]\s*/gi,(_,loc)=>{markerScene=String(loc).toUpperCase();changed=true;return ' '}).trim();
    }
    if(markerScene)setScene(markerScene);else{const inferred=inferSceneFromNarrative(narrative);if(inferred)setScene(inferred)}
