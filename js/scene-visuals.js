...
function ensureState(){
  ...
  if(!q)return s;
  const first=firstSceneId(q),locked=openingLockedScene(q);
...
function panel(){
  ...
  if(!p){p=document.createElement('figure');p.id='sceneVisual';p.className='scene-visual';p.innerHTML='<img id="sceneVisualImg" alt=""><figcaption id="sceneVisualCaption"></figcaption>'}
...
async function render(){
  await loadCatalog();installStyle();const p=panel();if(!p)return;
  const s=ensureState(),asset=assetFor(s.questId,s.sceneId);if(!asset){p.classList.remove('active');return}
  const img=document.getElementById('sceneVisualImg'),cap=document.getElementById('sceneVisualCaption');
  if(img&&img.getAttribute('src')!==asset.src)img.src=asset.src;if(img)img.alt=asset.label||'';if(cap)cap.textContent=asset.label||'';p.classList.add('active')
}

async function setScene(sceneId,questId=campaignQuest()){
  await loadCatalog();const q=questId,id=String(sceneId||'').toUpperCase(),locked=openingLockedScene(q);
  if(locked&&id!==locked)return false;
  if(!valid(q,id))return false;
  const previous=read();if(previous.questId===q&&previous.sceneId===id){await render();return true}
  write({questId:q,sceneId:id,updatedAt:new Date().toISOString()});await render();window.dispatchEvent(new CustomEvent('valedouro:scene-change',{detail:{questId:q,sceneId:id}}));return true
}
...
function inferSceneFromNarrative(text,q=campaignQuest()){
  if(openingLockedScene(q))return openingLockedScene(q);
  const t=normalizeText(text);if(!t)return null;const scenes=scenesFor(q);if(!scenes.length)return null;let best=null,bestScore=0;
  for(const scene of scenes){const words=normalizeText(scene.label).split(' ').filter(w=>w.length>=4);if(!words.length)continue;const score=words.reduce((n,w)=>n+(t.includes(w)?1:0),0)/words.length;if(score>bestScore){bestScore=score;best=scene.id}}
...
window.fetch=async function(input,init){
  ...
  try{
    await loadCatalog();const data=await res.clone().json();let changed=false,markerScene=null,narrative='';
    for(const field of ['reply','text'])if(typeof data?.[field]==='string'){narrative+=' '+data[field];data[field]=data[field].replace(/\s*\[\[SCENE:([A-Z]{3}-\d{3})\]\]\s*/gi,(_,id)=>{markerScene=String(id).toUpperCase();changed=true;return ' '}).trim()}
    const q=campaignQuest(),locked=openingLockedScene(q);
    if(locked)await setScene(locked,q);else if(markerScene)await setScene(markerScene);else{const inferred=inferSceneFromNarrative(narrative,q);if(inferred)await setScene(inferred,q)}
...
async function init(){await loadCatalog();await render()}if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',init);else init();
})();