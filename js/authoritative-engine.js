...
function authoritativeSnapshot(){
  ...
  return {
    policy:'WEBIA_STATE_IS_ABSOLUTE_TRUTH',
    questId:questId(),sceneId:sceneId(),
    ...
    rules:[
      ...
      'A player declaration describes intent, not automatic success. If uncertainty matters, request a roll before narrating the outcome.',
...
function classifyAction(raw){
  ...
  if(/(largo|solto|deixo|ponho).{0,30}escudo/.test(t)&&!risky)return {type:'drop_shield',deterministic:true};
  if(/(pego|recolho|apanho|recupero).{0,30}escudo/.test(t)&&!risky)return {type:'pick_shield',deterministic:true};
  if(/(saco|desembainho|empunho|guardo|embainho).{0,35}(espada|arma|arco)/.test(t)&&!risky)return {type:'ready_weapon',deterministic:true};
  if(/(disparo|ataco|atira|solto).{0,45}(arco|flecha)|(?:arco|flecha).{0,45}(disparo|ataco|atira)/.test(t))return {type:'ranged_attack',deterministic:false};
  return {type:'general',deterministic:false};
}
...
function deterministicFallback(type){
  if(type==='drop_shield')return 'Mestre: Você simplesmente solta o escudo. Ele cai ao chão ao seu lado, deixando sua mão livre. A ação não exige teste. O que você faz?';
  if(type==='pick_shield')return 'Mestre: Você recolhe o escudo e volta a mantê-lo pronto para uso. A ação não exige teste. O que você faz?';
  return 'Mestre: Você realiza a preparação simples que declarou, sem necessidade de teste. O que você faz?';
}

function secureDie(sides){
  sides=Math.max(2,Math.floor(Number(sides)||20));
  if(window.crypto?.getRandomValues){
    const max=Math.floor(0x100000000/sides)*sides,a=new Uint32Array(1);let x;
...
function countFromText(text){
  const t=normalize(text);let found=[];
  const re=/\b(dois|duas|tres|quatro|cinco|seis|sete|oito|nove|dez|\d+)\s+(bandidos?|inimigos?|arqueiros?|figuras encapuzadas?)\b/g;let m;
  while((m=re.exec(t))){const n=/^\d+$/.test(m[1])?Number(m[1]):numberWords[m[1]];if(n>=2)found.push(n)}
...
function hasAmmoContradiction(text,snapshot){const q=snapshot?.character?.inventory?.find(x=>x.ammunition)?.qty||0;if(q<=0)return false;return /(sem flechas|sem municao|aljava vazia|nao (?:tem|possui|ha) (?:mais )?flechas|acabaram? (?:as )?flechas)/.test(normalize(text))}
function rollMarker(text){return String(text||'').match(/\[\[ROLL:(FOR|DES|CON|INT|SAB|CAR):(\d+):([^\]]+)\]\]/i)}
function outcomeBeforeRoll(text){
  const m=rollMarker(text);if(!m)return false;
  const before=normalize(String(text).slice(0,String(text).indexOf(m[0])));
  return /(acerta|atinge|erra|falha|consegue|derruba|mata|cai ao chao|fica ferido|sofre dano|desarma|escapa|vence|perde o alvo)/.test(before);
}
...
function masterDirective(action,snapshot){
  return [
    'MOTOR AUTORITATIVO DO WEBIA — OBRIGATÓRIO.',
    'Você é o Mestre narrativo, não a fonte de verdade mecânica.',
    'Use authoritative_state como verdade absoluta. Nunca contradiga inventário, munição, equipamento, CA, PV ou fatos de encontro.',
    action.deterministic?'A ação atual é DETERMINÍSTICA e não exige rolagem. Narre sua realização sem pedir [[ROLL]].':'Só peça [[ROLL]] quando existir incerteza relevante e uma consequência real para falha.',
    'Se pedir [[ROLL]], NÃO narre sucesso, falha, acerto, erro, dano ou consequência antes de receber o resultado da rolagem.',
    'Não invente inimigos extras nem reforços. Uma contagem de hostis já estabelecida na cena permanece fixa até um evento explícito do estado.',
    'Falhas necessárias à progressão devem produzir consequência e avanço, nunca loops de repetição da mesma perícia.',
    'Não invente números exatos ausentes do estado/cânone. Se um NPC não sabe uma quantidade, diga que não sabe ao certo.'
  ].join(' ')
}

window.fetch=async function(input,init){
  const url=typeof input==='string'?input:(input?.url||''),ai=(typeof AI_ENDPOINT!=='undefined'?AI_ENDPOINT:null);
  if(url!==ai||!init?.body||String(init.method||'GET').toUpperCase()!=='POST')return nativeFetch(input,init);
  let body,action={type:'general',deterministic:false},snapshot;
  try{
    body=JSON.parse(init.body);action=classifyAction(actionText(body));snapshot=authoritativeSnapshot();
    body.state={...(body.state||{}),authoritative_state:snapshot};
    body.world={...(body.world||{}),authoritative_engine:masterDirective(action,snapshot)};
    body.action=masterDirective(action,snapshot)+' '+String(body.action||'');
  }catch(e){console.warn('Motor autoritativo: não foi possível preparar contexto',e);return nativeFetch(input,init)}

  let res=await nativeFetch(input,{...init,body:JSON.stringify(body)});
  try{
    let data=await res.clone().json();let text=String(data?.reply??data?.text??'');
    const enc=currentEncounter(),count=countFromText(text),existing=enc.visibleHostiles;
    let violation=null;
    ...
    else if(hasAmmoContradiction(text,snapshot))violation='Você contradisse a munição autoritativa. Reescreva usando exatamente a quantidade de munição fornecida no authoritative_state.';
    ...
    if(violation){
      const retry={...body,action:`CORREÇÃO OBRIGATÓRIA: ${violation} Não acrescente novos fatos. `+masterDirective(action,snapshot)};
    ...
    if(finalText.includes('[[QUEST_COMPLETE]]')){
      try{await window.ValeDouroCampaign?.completeCurrent?.()}catch(e){console.warn('Conclusão de Quest não aplicada',e)}
      const clean=finalText.replace(/\s*\[\[QUEST_COMPLETE\]\]\s*/g,' ').trim();
      const d2=await res.clone().json();res=jsonResponse(res,replaceFields(d2,clean));
    }
  }catch(e){console.warn('Motor autoritativo: validação de resposta não aplicada',e)}
  return res;
};

// Pré-processa ações determinísticas e consumo real de munição antes da narração.
const originalAct=window.act;
if(typeof originalAct==='function')window.act=async function(){
...
window.addEventListener('valedouro:scene-change',()=>{/* encontros são automaticamente separados por chave de cena */});

window.ValeAuthority={
  read:auth,snapshot:authoritativeSnapshot,classifyAction,secureDie,
  reset(){localStorage.removeItem(KEY)},
  diceLog(){try{return JSON.parse(localStorage.getItem(DICE_KEY)||'[]')}catch{return []}},
  auditDice(n=10000,sides=20){n=Math.max(100,Math.min(200000,Number(n)||10000));const counts=Array(sides).fill(0);for(let i=0;i<n;i++)counts[secureDie(sides)-1]++;const expected=n/sides;return {n,sides,expected,counts,min:Math.min(...counts),max:Math.max(...counts),spread:(Math.max(...counts)-Math.min(...counts))/expected}}
};
})();